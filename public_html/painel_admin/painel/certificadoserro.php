...
//Quantia total não tranferida
$result_estoques3 = "SELECT sum(quantia) FROM saque where status = 'invalido'";
...
$result_estoque = "SELECT * FROM saque where status = 'invalido'";
...
$result_estoques = "SELECT * FROM saque where status = 'invalido' limit $inicio, $quantidade_pg";
$resultado_estoques = mysqli_query($conn, $result_estoques);
...
if (isset($_GET["codigo"])) {
    $tranferencia->depositado($_GET["codigo"]);
}else if(isset($_GET["codigo3"])) {
    $tranferencia->pendente($_GET["codigo3"]);
}//else if(isset($_GET["codigo2"])){
    //$tranferencia->devolucao($_GET["codigo2"]);
    //$cmdzao = $_GET["cmd"];
    //header("location:erro.php?tx=".urlencode($cmdzao)."");
//}
?>
<!DOCTYPE html>
<html lang="pt">
...
<nav class="navbar navbar-inverse">
  ...
  <center>
        ...
        <div class="col-md-8">
            <center>
            <h3><b><i class='fa fa-close'></i> Erro no Envio do Certificado</b></h3></center>
       ...
       <?php while ($rows_estoques = mysqli_fetch_assoc($resultado_estoques)){ 
        ...
        ?>
                <tr>
                    ...
                    <td><?php echo $row_estoques['nome'];?></td>
                    <td><?php echo $row_estoques['email'];?></td>
                    <td><?php echo $row_estoques['telefone'];?></td>

                    <td>
                        
                        <a href="certificadoserro.php?codigo=<?php echo $rows_estoques['codigo']; ?>" class="btn btn-success">Depositado</a>
                        <a href="certificadoserro.php?codigo3=<?php echo $rows_estoques['codigo']; ?>" class="btn btn-danger">Pendente</a>
                        <!--<a href="erro.php?codigo2=<?php echo $rows_estoques['codigo']; ?>" class="btn btn-info">Devolução</a>-->
                    </td>
